...
import { readFileSync, writeFileSync, unlinkSync, existsSync, mkdirSync, appendFileSync } from 'fs';
...
const DEBUG_LOG = resolve(RUNTIME_DIR, 'cdp-debug.log');
const DEBUG = process.env.CDP_DEBUG === '1';

function debugLog(...parts) {
  if (!DEBUG) return;
  const line = `[${new Date().toISOString()} pid=${process.pid}] ${parts.map(part => {
    if (typeof part === 'string') return part;
    try { return JSON.stringify(part); } catch { return String(part); }
  }).join(' ')}\n`;
  try { appendFileSync(DEBUG_LOG, line, { mode: 0o600 }); } catch {}
}
...
function getWsUrl() {
  ...
  const macBrowsers = [
    ...
    'Chromium', 'BraveSoftware/Brave-Browser', 'Microsoft Edge', 'net.imput.helium'
  ];
...
class CDP {
  ...
  async connect(wsUrl) {
    return new Promise((res, rej) => {
      ...
      this.#ws.onerror = (e) => {
        debugLog('ws error', e?.message || e?.type || 'unknown');
        rej(new Error('WebSocket error: ' + (e.message || e.type)));
      };
      this.#ws.onclose = (ev) => {
        debugLog('ws close', { code: ev?.code, reason: ev?.reason, wasClean: ev?.wasClean });
      ...
      this.#ws.onmessage = (ev) => {
        ...
        if (msg.id && this.#pending.has(msg.id)) {
          ...
          if (msg.error) {
            ...
            debugLog('recv result', { id: msg.id, result: msg.result });
            resolve(msg.result);
          }
        } else if (msg.method) {
          debugLog('event', { method: msg.method, sessionId: msg.sessionId, params: msg.params || {} });
          if (this.#eventHandlers.has(msg.method)) {
  ...
  send(method, params = {}, sessionId) {
    ...
    return new Promise((resolve, reject) => {
      ...
      debugLog('send', { id, method, sessionId, params });
...
async function snapshotStr(cdp, sid, compact = false) {
  await cdp.send('Accessibility.enable', {}, sid);

  const { node: root } = await cdp.send('Accessibility.getRootAXNode', {}, sid);
  const nodesById = new Map([[root.nodeId, root]]);
  ...
  while (queue.length > 0) {
    ...
    if (!node?.childIds?.length || expanded.has(node.nodeId)) continue;
    expanded.add(node.nodeId);
    const { nodes: childNodes } = await cdp.send('Accessibility.getChildAXNodes', { id: node.nodeId }, sid);
    for (const child of childNodes) {
      const existing = nodesById.get(child.nodeId);
      nodesById.set(child.nodeId, existing ? { ...existing, ...child } : child);
      if (child.parentId) {
        if (!childrenByParent.has(child.parentId)) childrenByParent.set(child.parentId, []);
        const arr = childrenByParent.get(child.parentId);
        if (!arr.some(n => n.nodeId === child.nodeId)) arr.push(nodesById.get(child.nodeId));
      }
    }
    for (const childId of node.childIds) {
      const child = nodesById.get(childId);
  ...
  visit(root, 0);
  for (const node of nodesById.values()) visit(node, 0);
...
async function runDaemon(targetId) {
  ...
  function shutdown() {
    if (!alive) return;
    alive = false;
    debugLog('daemon shutdown', { targetId, sessionId, socket: sp });
    server.close();
  ...
  cdp.onEvent('Target.targetDestroyed', (params) => {
    debugLog('targetDestroyed', params);
    if (params.targetId === targetId) shutdown();
  });
  cdp.onEvent('Target.detachedFromTarget', (params) => {
    debugLog('detachedFromTarget', { params, currentSessionId: sessionId });
    if (params.sessionId === sessionId) sessionId = null;
  });
  cdp.onClose(() => {
    debugLog('cdp onClose -> shutdown');
    shutdown();
  });
  ...
  async function handleCommand({ cmd, args }) {
    ...
    try {
      let result;
      if (cmd !== 'list' && cmd !== 'list_raw' && cmd !== 'stop') {
      ...
      if (message.includes('Session with given id not found') || message.includes('No session with given id')) {
        try {
          sessionId = null;
          await ensureAttached();
          let retryResult;
          switch (cmd) {
            case 'snap': case 'snapshot': retryResult = await snapshotStr(cdp, sessionId, true); break;
            case 'eval': retryResult = await evalStr(cdp, sessionId, args[0]); break;
            case 'shot': case 'screenshot': retryResult = await shotStr(cdp, sessionId, args[0], targetId); break;
            case 'html': retryResult = await htmlStr(cdp, sessionId, args[0]); break;
            case 'nav': case 'navigate': retryResult = await navStr(cdp, sessionId, args[0]); break;
            case 'net': case 'network': retryResult = await netStr(cdp, sessionId); break;
            case 'click': retryResult = await clickStr(cdp, sessionId, args[0]); break;
            case 'clickxy': retryResult = await clickXyStr(cdp, sessionId, args[0], args[1]); break;
            case 'type': retryResult = await typeStr(cdp, sessionId, args[0]); break;
            case 'loadall': retryResult = await loadAllStr(cdp, sessionId, args[0], args[1] ? parseInt(args[1]) : 1500); break;
            case 'evalraw': retryResult = await evalRawStr(cdp, sessionId, args[0], args[1]); break;
            default: throw e;
          }
