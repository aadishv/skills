...
3. Take advantage of HTML and CSS; don't use Tailwind unless the codebase prescribes it. Use React etc. if the codebase prescribes it or if the site is sufficiently interactive. In most cases, you should be able to get away with a `<script type="module" src="./index.ts"></script>` or similar (relative paths work, and tsx files too) - then, you can simply use `bun /path/to/file.html` which will handle the TypeScript(X) compilation and bundling for you.
